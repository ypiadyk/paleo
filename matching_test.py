...
def map_to(p, off_0, off_1, ref_O, ref_B, O, B, mtx, plot=False):
...
def normalize_image(img, m, s):
    img = 127 + 70 * (img.astype(np.float32) - m) / s
    img = np.maximum(0, np.minimum(img, 255))
...
def do_matching(data_path, cam_calib, save=None, plot=False, save_figures=None, **kw):
    ...
    ref, min_h, max_h, pad = 52, 19, 24, 15
    # roi = (2900, 1600, 3700, 1900)
    roi = (2200, 1600, 3900, 2300)
    px, py = np.meshgrid(np.linspace(roi[0], roi[2], 7), np.linspace(roi[1], roi[3], 5))#, indexing="ij")
    ...
    crop = imgs[ref][roi[1]:roi[3], roi[0]:roi[2], :]
    crop = np.average(crop, axis=2)
    cm, cs = np.mean(crop), np.std(crop)
    print(cm, cs)

    imgs[ref] = normalize_image(imgs[ref], cm, cs)
    ...
    for i in range(len(imgs)):
        ...
        imgs[i] = normalize_image(imgs[i], cm, cs)

        s0, s1 = map_to(all_p, min_h, max_h, O[ref], B[ref], O[i], B[i], cam_calib["new_mtx"], plot=(plot and (i == 1)))
    ...
    if plot:
        ...
        for i in range(7*5):
            ...
            for mi, match in all_matches[i].items():
                ...
                x = np.linspace(min_h, max_h, match.shape[0])
                ...
                resolution = float(match.shape[0])

                xs.append(x[am])
                ms.append(match[am])
                ws.append(contrast * priority * resolution)
